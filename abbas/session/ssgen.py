...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as shahm:
            print("Generating a string session for •shahm•")
            try:
                shahm.send_message(
                    "me",
                    f"**- هـذا هو كـود سيشـن تيليثـون الخـاص بـك ☑️**:\n\n`{shahm.session.save()}`\n\n**- لا تشـارك هـذا الكـود مع اي احـد ⚠️؟!**\n**- فقـط قم بنسخـه ولصقـه في مربعـات التنصيب 🛃**\n\n**- سـورس شـهــم : @shahm50**",
                )
                print(
        ...
        print(er)
        print("If you think It as a Bug, Report to @shahm41\n\n")
...
def pyro_session():
    ...
    with Client(
        name="shahm", api_id=API_ID, api_hash=API_HASH, in_memory=True
    ) as pyro:
