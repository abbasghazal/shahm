...
@shahm.rep_cmd(pattern=f"للكل(?: |$)(.*)", groups_only=True)
...
@shahm.rep_cmd(pattern="ايقاف للكل", groups_only=True)
...
async def malath(event):
    ...
    for user in users:
        ...
        if abbas.text:
            try:
                user_entity = await shahm.get_entity(user)
                if user_entity.bot or user_entity.deleted:
                    continue
                await shahm.send_message(user_entity.id, zilzal, link_preview=False)
                success += 1
            ...
            try:
                ...
                if user_entity.bot or user_entity.deleted:
                    continue
                await shahm.send_file(user_entity.id, zilzal.media, caption=zilzal.text)
                success += 1
            except UserAdminInvalidError:
                pass
            except Exception as e:
