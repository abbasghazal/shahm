...
@shahm.rep_cmd(pattern="(تقليد|ازعاج)$")
...
@shahm.rep_cmd(pattern="(الغاء تقليد|ايقاف التقليد|ايقاف تقليد|الغاء الازعاج|ايقاف الازعاج|ايقاف ازعاج|الغاء ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@shahm.rep_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@shahm.rep_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@shahm.rep_cmd(incoming=True, edited=False)
