...
@shahm1.rep_cmd(pattern="(تقليد|ازعاج)$")
...
@shahm1.rep_cmd(pattern="(الغاء تقليد|ايقاف التقليد|ايقاف تقليد|الغاء الازعاج|ايقاف الازعاج|ايقاف ازعاج|الغاء ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@shahm1.rep_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@shahm1.rep_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@shahm1.rep_cmd(incoming=True, edited=False)
