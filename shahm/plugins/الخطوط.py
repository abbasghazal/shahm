...
@shahm1.on(admin_cmd(pattern="(خط الغامق|خط غامق)"))
...
@shahm1.on(admin_cmd(pattern="(خط رمز|خط الرمز)"))
async def btext(event):
...
@shahm1.on(admin_cmd(pattern="(خط المشطوب|خط مشطوب)"))
async def btext(event):
...
@shahm1.on(admin_cmd(pattern="(خط مائل|الخط المائل)"))  
async def btext(event):
...
@shahm1.on(events.NewMessage(outgoing=True))
