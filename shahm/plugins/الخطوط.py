...
@shahm.on(admin_cmd(pattern="(خط الغامق|خط غامق)"))
...
@shahm.on(admin_cmd(pattern="(خط رمز|خط الرمز)"))
async def btext(event):
...
@shahm.on(admin_cmd(pattern="(خط المشطوب|خط مشطوب)"))
async def btext(event):
...
@shahm.on(admin_cmd(pattern="(خط مائل|الخط المائل)"))  
async def btext(event):
