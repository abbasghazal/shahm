...
@shahm.rep_cmd(pattern="الذاتيه")
async def cmd(abbas):
    await edit_or_reply(abbas, abbasSelf_cmd)

@shahm.rep_cmd(pattern=f"{POSC}(?: |$)(.*)")
async def oho(event):
    ...
    await shahm.send_file("me", pic, caption=f"**⎉╎تم حفـظ الصـورة الذاتيـه .. بنجـاح ☑️𓆰**")
    await event.delete()

@shahm.rep_cmd(pattern="(تفعيل الذاتيه|تفعيل الذاتية)")
...
@shahm.rep_cmd(pattern="(تعطيل الذاتيه|تعطيل الذاتية)")
...
@shahm.on(events.NewMessage(func=lambda e: e.is_private and (e.photo or e.video) and e.media_unread))
...
#Code For T.me/shahm41
@shahm.rep_cmd(pattern="إعلان (\d*) ([\s\S]*)")
async def selfdestruct(destroy):
    rep = ("".join(destroy.text.split(maxsplit=1)[1:])).split(" ", 1)
    message = rep[1]
    ttl = int(rep[0])
