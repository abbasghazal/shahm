...
@shahm.rep_cmd(pattern="(ضع الاشتراك خاص|وضع الاشتراك خاص)(?:\s|$)([\s\S]*)")
async def _(event):
...
@shahm.rep_cmd(pattern="(ضع الاشتراك كروب|وضع الاشتراك كروب)(?:\s|$)([\s\S]*)")
async def _(event):
    if input_str := event.pattern_match.group(2):
        try:
            p = await event.client.get_entity(input_str)
...
@shahm.rep_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
    ...
    if gvarstatus ("sub_private"):
        try:
            ...
            try:
                ...
                return await shahm.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ \n{r}**")
            url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
            req = requests.get(url)
            reqt = req.text
            if "chat not found" in reqt:
                mb = await shahm.tgbot.get_me()
                mb = mb.username
                await shahm.tgbot.send_message(BOTLOG_CHATID, f"**⎉╎البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري ؟!**")
                return
            if "bot was kicked" in reqt:
                mb = await shahm.tgbot.get_me()
                mb = mb.username
                await shahm.tgbot.send_message(BOTLOG_CHATID, "**⎉╎البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته؟!**")
                return
            if "not found" in reqt:
                try:
                    c = await shahm.get_entity(ch)
                    chn = c.username
                    if c.username == None:
                        ra = await shahm.tgbot(ExportChatInviteRequest(ch))
                        chn = ra.link
                    ...
                    await shahm.tgbot.send_message(BOTLOG_CHATID, f"- خطـأ \n{er}")
            if "left" in reqt:
                try:
                    c = await shahm.get_entity(ch)
                    chn = c.username
            ...
            if "error_code" in reqt:
                await shahm.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ غير معروف قم باعادة توجيه الرسالة ل@shahm41 لحل المشكلة\n{reqt}**")
