...
from shahm import BOTLOG_CHATID
...
@shahm1.rep_cmd(pattern="(ضع الاشتراك خاص|وضع الاشتراك خاص)(?:\s|$)([\s\S]*)")
async def _(event):
...
@shahm1.rep_cmd(pattern="(ضع الاشتراك كروب|وضع الاشتراك كروب)(?:\s|$)([\s\S]*)")
async def _(event):
    if input_str := event.pattern_match.group(2):
        try:
            p = await event.client.get_entity(input_str)
...
@shahm1.rep_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
async def supc(event):  # shahm - abbas
    chat = await event.get_chat()
    rep_dev = (6848908141, 6848908141)
    abbas = (await event.get_sender()).id
    if abbas in rep_dev:
    	return
    if chat.bot:
    	return
    if gvarstatus ("sub_private"):
        try:
            ...
            try:
                ...
                return await shahm1.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ \n{r}**")
            url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
            req = requests.get(url)
            reqt = req.text
            if "chat not found" in reqt:
                mb = await shahm1.tgbot.get_me()
                mb = mb.username
                await shahm1.tgbot.send_message(BOTLOG_CHATID, f"**⎉╎البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري ؟!**")
                return
            if "bot was kicked" in reqt:
                mb = await shahm1.tgbot.get_me()
                mb = mb.username
                await shahm1.tgbot.send_message(BOTLOG_CHATID, "**⎉╎البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته؟!**")
                return
            if "not found" in reqt:
                try:
                    c = await shahm1.get_entity(ch)
                    chn = c.username
                    if c.username == None:
                        ra = await shahm1.tgbot(ExportChatInviteRequest(ch))
                        chn = ra.link
                    ...
                    await shahm1.tgbot.send_message(BOTLOG_CHATID, f"- خطـأ \n{er}")
            if "left" in reqt:
                try:
                    c = await shahm1.get_entity(ch)
                    chn = c.username
            ...
            if "error_code" in reqt:
                await shahm1.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ غير معروف قم باعادة توجيه الرسالة ل@shahm41 لحل المشكلة\n{reqt}**")
