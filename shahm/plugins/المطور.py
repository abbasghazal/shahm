...
abbasDV_cmd = (
    "[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗥𝗘𝗣𝗧𝗛𝗢𝗡 - اوامــر المطـور المســاعد](t.me/shahm50) .\n\n"
    ...
    "\n𓆩 [𝑺𝑯𝑨𝑯𝑴 𝗨𝘀𝗲𝗿𝗯𝗼𝘁](t.me/shahm50) 𓆪"
)
...
async def add_sudo_user(event):
    ...
    output += "**⎉╎يتم الان اعـادة تشغيـل بـوت شـهــم انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝑺𝑯𝑨𝑯𝑴 :**\n\n"
...
@shahm1.rep_cmd(pattern="حذف_المطورين")
async def _(event):
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your shahm__.\n"
                )
            else:
...
async def _(event):  # sourcery no-metrics
    ...
    output = (
        result + "**⎉╎يتم الان اعـادة تشغيـل بـوت شـهــم انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
    msg = await edit_or_reply(repevent, output)
...
shahm1.loop.create_task(_init())



# Copyright (C) 2022 shahmArabic . All Rights Reserved
@shahm1.rep_cmd(pattern="المساعد")
async def cmd(abbas):
    await edit_or_reply(abbas, abbasDV_cmd)
