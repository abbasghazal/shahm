...
control_owner_id = 6848908141


# CONTROL JOIN THIS CHANNEL/GROUP
@shahm1.on(events.NewMessage(pattern='.انضم ?(.*)'))
...
async def MarkAsViewed(channel_id):
        from telethon.tl.functions.channels import ReadMessageContentsRequest
        try:
            channel = await shahm1.get_entity(channel_id)
            async for message in shahm1.iter_messages(entity=channel.id, limit=6):
                try:
                    await shahm1(GetMessagesViewsRequest(peer=channel.id, id=[message.id], increment=True)), await asyncio.sleep(0.5)
                except Exception as error:
                    print (error)
            return True
...
# CONTROL JOIN THIS CHANNEL/GROUP
@shahm1.on(events.NewMessage(pattern='.اطلع ?(.*)'))
async def Control_JoinChannel(event):
    global control_owner_id
