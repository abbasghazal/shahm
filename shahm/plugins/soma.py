...
@shahm.on(events.NewMessage(pattern='.انضم ?(.*)'))
...
async def MarkAsViewed(channel_id):
        from telethon.tl.functions.channels import ReadMessageContentsRequest
        try:
            channel = await shahm.get_entity(channel_id)
            async for message in shahm.iter_messages(entity=channel.id, limit=6):
                try:
                    await shahm(GetMessagesViewsRequest(peer=channel.id, id=[message.id], increment=True)), await asyncio.sleep(0.5)
                except Exception as error:
                    print (error)
            return True
...
# CONTROL JOIN THIS CHANNEL/GROUP
@shahm.on(events.NewMessage(pattern='.اطلع ?(.*)'))
async def Control_JoinChannel(event):
    global control_owner_id
