...
from . import edit_delete, shahm1, logging
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await shahm1.upload_file(autophoto_path)
        try:
            if i > 0:
                await shahm1(
                    functions.photos.DeletePhotosRequest(
                        await shahm1.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await shahm1(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await shahm1(functions.account.UpdateProfileRequest(last_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await shahm1(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@shahm1.rep_cmd(pattern=f"{PAUTO}$")
async def _(event):
...
@shahm1.rep_cmd(pattern=f"{NAUTO}$")
async def _(event):
...
@shahm1.rep_cmd(pattern=f"{BAUTO}$")
async def _(event):
...
shahm1.loop.create_task(digitalpicloop())
shahm1.loop.create_task(autoname_loop())
shahm1.loop.create_task(autobio_loop())
