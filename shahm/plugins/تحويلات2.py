#shahm ®
#الملـف حقـوق وتعديـل بـاقـر ⤶ @shahm41 خاص بسـورس ⤶ 𝑺𝑯𝑨𝑯𝑴
...
@shahm1.rep_cmd(pattern="دائري ?((-)?s)?$")
...
@shahm1.rep_cmd(pattern="ملصق متحرك(?: |$)(.*)")
...
@shahm1.rep_cmd(pattern="لمتحركه(?: |$)((-)?(r|l|u|d|s|i)?)$")
...
@shahm1.rep_cmd(pattern="متحرك ?([0-9.]+)?$")
async def _(event):
    reply = await event.get_reply_message()
