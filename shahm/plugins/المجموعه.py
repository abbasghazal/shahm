...
@shahm1.rep_cmd(pattern="اضافه ([\s\S]*)", groups_only=True)
async def get_users(event):
    ...
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await shahm1(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await shahm.edit(
                f"**❈╎جـارِ الإضافـة . . .**\n\n**❈╎اضيف** `{s}` \n**❈╎خطـأ بـ اضافـة** `{f}` \n\n**❈╎آخـر خطـأ:** `{error}`"
            )
...
async def tagall(event):
  ...
  if mode == "by_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in shahm1.iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await shahm1.send_message(event.chat_id, f"{usrtxt}\n\n- {msg}")
  ...
  if mode == "by_reply":
    ...
    async for usr in shahm1.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await shahm1.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""
...
@shahm1.rep_cmd(pattern="ايقاف التاك?(.*)")
...
@shahm1.rep_cmd(pattern="تجميع الاعضاء$")
...
@shahm1.rep_cmd(pattern="اضف الاعضاء$")
...
@shahm1.rep_cmd(pattern="رفع الحظر ?(.*)")
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
...
@shahm1.rep_cmd(pattern=r"مسح المحظورين(.*)")
async def _(event):
...
@shahm1.rep_cmd(pattern=r"المحذوفين ?([\s\S]*)")
...
@shahm1.rep_cmd(pattern="احصائياتي$")
