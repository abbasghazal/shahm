...
shahm_RIGHTS = ChatBannedRights(until_date=None, view_messages=True, send_messages=True, send_media=True, send_stickers=True, send_gifs=True, send_games=True, send_inline=True, embed_links=True)
...
CHANNELS_STR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - احصـائيـات جميـع القنـوات** 𓆪\n\n"
CHANNELS_ADMINSTR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - احصـائيـات جميـع القنـوات اشـراف** 𓆪\n\n"
CHANNELS_OWNERSTR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - احصـائيـات جميـع القنـوات ملكيـة** 𓆪\n\n"
GROUPS_STR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - احصـائيـات جميـع المجمـوعـات** 𓆪\n\n"
GROUPS_ADMINSTR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - احصـائيـات جميـع المجمـوعـات اشـراف** 𓆪\n\n"
GROUPS_OWNERSTR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - احصـائيـات جميـع المجمـوعـات ملكيـة** 𓆪\n\n"
# =========================================================== #
#                                                           shahm                                                               #
# =========================================================== #
...
async def ban_user(chat_id, i, rights):
    try:
        await shahm(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
@shahm.rep_cmd(pattern="اضافه ([\s\S]*)", groups_only=True)
async def get_users(event):
    legen_ = event.text[10:]
    shahm_chat = legen_.lower
    restricted = ["@shahm50_support", "@shahm50_support"]
    shahm = await edit_or_reply(event, f"**❈╎جـارِ اضافـه الاعضـاء مـن  ** {legen_}   **⅏ . . .**")
    if shahm_chat in restricted:
    ...
    if not sender.id == me.id:
        await shahm.edit("**❈╎ جـارِ إتمـام العمليـة إنتظـر ⅏ . . .**")
    else:
        await shahm.edit("**❈╎ جـارِ إتمـام العمليـة إنتظـر ⅏ . . .**")
    if event.is_private:
        return await shahm.edit("- لا يمكنك اضافه الاعضاء هنا")
    s = 0
    f = 0
    error = "None"
    await shahm.edit(
    ...
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await shahm(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await shahm.edit(
                f"**❈╎جـارِ الإضافـة . . .**\n\n**❈╎اضيف** `{s}` \n**❈╎خطـأ بـ اضافـة** `{f}` \n\n**❈╎آخـر خطـأ:** `{error}`"
            )
...
async def stats(event):  # sourcery no-metrics
    ...
    response = f"𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - احصـائيـات {full_name}** 𓆪\n"
...
async def tagall(event):
  ...
  if mode == "by_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in shahm.iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await shahm.send_message(event.chat_id, f"{usrtxt}\n\n- {msg}")
  ...
  if mode == "by_reply":
    ...
    async for usr in shahm.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await shahm.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""
...
@shahm.rep_cmd(pattern="ايقاف التاك?(.*)")
...
@shahm.rep_cmd(pattern="تجميع الاعضاء$")
...
@shahm.rep_cmd(pattern="اضف الاعضاء$")
...
async def _(event):  # sourcery no-metrics
    ...
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if i.bot:
            ...
            n += 1
    if input_str:
        required_string = """𓆰 𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - معـلومـات المجمـوعــه** 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻 
...
𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻"""
        ...
        await sleep(5)
    await et.edit(
        """𓆰 𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - معـلومـات المجمـوعــه** 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻
⪼ **العدد ↫ {} **مستخدماً
⪼ **الحسابات المحذوفه ↫** {}
⪼ **اخر ظهور منذ زمن طويل ↫** {}
⪼ **اخر ظهور منذ شهر ↫** {}
...
@shahm.rep_cmd(pattern="رفع الحظر ?(.*)")
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
...
@shahm.rep_cmd(pattern=r"المحذوفين ?([\s\S]*)")
...
@shahm.rep_cmd(pattern="احصائياتي$")
async def count(event):
    ...
    result += f"𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝑺𝑯𝑨𝑯𝑴 **- 🝢 - احصـائيـات الحسـاب** 𓆪\n"
