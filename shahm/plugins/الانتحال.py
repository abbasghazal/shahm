...
from . import ALIVE_NAME, BOTLOG, BOTLOG_CHATID, shahm, edit_delete, get_user_from_event
...
@shahm.rep_cmd(pattern="نسخ(?:\s|$)([\s\S]*)")
async def _(event):
...
@shahm.rep_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@shahm.rep_cmd(pattern=f"{ANTHAL}$")
