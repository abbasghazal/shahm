...
from . import ALIVE_NAME, BOTLOG, BOTLOG_CHATID, shahm1, edit_delete, get_user_from_event
...
@shahm1.rep_cmd(pattern="نسخ(?:\s|$)([\s\S]*)")
async def _(event):
...
@shahm1.rep_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@shahm1.rep_cmd(pattern=f"{ANTHAL}$")
