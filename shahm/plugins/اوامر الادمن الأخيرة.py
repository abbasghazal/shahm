...
async def Hussein(event):
    ...
    if sender_id != 6848908141:
        if isinstance(event.message.entities, list) and any(isinstance(entity, MessageEntityCustomEmoji) for entity in event.message.entities):
            ...
            theshahm_entity = await shahm1.get_entity(sender.id)
...
shahm = False
async def shahm_nshr(shahm1, sleeptimet, chat, message, seconds):
    global shahm
    shahm = True
    while shahm:
        if message.media:
            sent_message = await shahm1.send_file(chat, message.media, caption=message.text)
        else:
            sent_message = await shahm1.send_message(chat, message.text)
        await asyncio.sleep(sleeptimet)

@shahm1.rep_cmd(pattern="انشر")
async def abbas(event):
    ...
    global shahm
    shahm = True
    await shahm_nshr(shahm1, sleeptimet, chat, message, seconds)
@shahm1.rep_cmd(pattern="ايقاف (النشر|نشر)")
