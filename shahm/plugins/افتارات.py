...
@shahm1.rep_cmd(pattern="ستوري انمي$")
async def _(event):
...
@shahm1.rep_cmd(pattern="ري اكشن$")
async def _(event):
    zzevent = await edit_or_reply(event, "**╮•⎚ جـارِ تحميـل الرياكشـن ...**")
    try:
        zedre = [
...
@shahm1.rep_cmd(pattern="ولد انمي$")
async def _(event):
...
@shahm1.rep_cmd(pattern="بنت انمي$")
async def _(event):
    zzevent = await edit_or_reply(event, "**╮ - جـارِ تحميـل الآفتـار ...𓅫╰**")
    try:
        zedph = [
