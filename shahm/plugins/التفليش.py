...
@shahm.rep_cmd(pattern=r"اطردني(.*)")
async def kickme(leave):
    await leave.edit("**⎉╎جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@shahm.rep_cmd(pattern=r"مغادره(.*)")
async def banme(leave):
    await leave.edit("**⎉╎جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@shahm.rep_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**⎉╎البـوت المسـاعد الخـاص بك هـو** \n {TG_BOT_USERNAME}")

@shahm.rep_cmd(pattern="حالتي ?(.*)")
async def rep(event):
...
@shahm.on(events.NewMessage(pattern="منصب؟"))
async def _(event):
...
@shahm.on(events.NewMessage(pattern="منو عمك؟"))
async def _(event):
    user = await event.get_sender()
    rep_dev = (6848908141, 6848908141)
    if user.id in rep_dev:
...
@shahm.rep_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
        await shahm(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in shahm.iter_participants(chat_id):
        ...
        await shahm.send_message(chat_id, usrtxt)
...
@shahm.rep_cmd(pattern="حظر_الكل$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@shahm.rep_cmd(pattern="كتم_الكل$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@shahm.rep_cmd(pattern="الغاء التفليش", groups_only=True)
...
@shahm.rep_cmd(pattern="ايقاف التفليش", groups_only=True)
async def unbanbot(event):
    if not event.chat_id in spam_chats:
        return await event.edit("**- لاتوجـد عمليـة تفليـش هنـا لـ إيقافـها ؟!**")
    else:
