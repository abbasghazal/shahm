...
def remove_plugin(shortname):
    ...
    try:
        name = f"shahm.plugins.{shortname}"
        for i in reversed(range(len(shahm1._event_builders))):
            ev, cb = shahm1._event_builders[i]
            if cb.__module__ == name:
                del shahm1._event_builders[i]
