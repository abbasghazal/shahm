...
from shahm import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID

from ..Config import Config
from ..core.logger import logging
from ..core.session import shahm1
...
Rep_Dev = (6848908141)
rchannel = {"@shahm50", "@shahm50n", "@shahm50_up", "@shahm50_vars", "@shahm50_support", "@shahm50_cklaish", "@shahm1", "@xxfir", "@shahm50_help", "@roger21v", "@Devs_shahm"}
...
async def autovars(): #Code by T.me/shahm41
...
async def autoname(): #Code by T.me/shahm41
    if gvarstatus("ALIVE_NAME"):
        return
    await bot.start()
    await asyncio.sleep(15)
    LOGS.info("جـارِ اضافة فـار الاسـم التلقـائـي .. انتظـر قليـلاً")
    abbas = await bot.get_me()
    rrname = f"{abbas.first_name} {abbas.last_name}" if abbas.last_name else f"{abbas.first_name}"
    tz = Config.TZ
...
async def setup_bot():
    """
    To set up bot for shahm
    """
    try:
        await shahm1.connect()
        config = await shahm1(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == shahm1.session.server_address:
                if shahm1.session.dc_id != option.id:
                    LOGS.warning(
                        f"ايـدي DC ثـابت فـي الجلسـة مـن {shahm1.session.dc_id}"
                        f" الـى {option.id}"
                    )
                shahm1.session.set_dc(option.id, option.ip_address, option.port)
                shahm1.session.save()
                break
        bot_details = await shahm1.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await shahm1.start(bot_token=Config.TG_BOT_USERNAME)
        shahm1.me = await shahm1.get_me()
        shahm1.uid = shahm1.tgbot.uid = utils.get_peer_id(shahm1.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(shahm1.me)
    except Exception as e:
        LOGS.error(f"كـود تيرمكس - {str(e)}")
        sys.exit()


async def mybot(): #Code by T.me/shahm41
    abbas = bot.me.first_name
    Taiba = bot.uid
    ba_qir = f"[{abbas}](tg://user?id={Taiba})"
    f"ـ {ba_qir}"
    f"•⎆┊هــذا البــوت خــاص بـ {ba_qir} يمكـنك التواصــل معـه هـنا 🧸♥️"
    babot = await shahm1.tgbot.get_me()
    ...
    if bot_name.startswith("مسـاعـد"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "شـهــم")
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_file("@BotFather", "shahm/abbas/shahm3.jpg")
            await asyncio.sleep(3)
            await bot.send_message("@BotFather", "/setabouttext")
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت شـهــم المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name}")
            await asyncio.sleep(3)
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.shahm1BLOGO = await shahm1.tgbot.send_file(
                BOTLOG_CHATID,
                "https://graph.org/file/f367d5a4a6bf1fbfc99b9.mp4",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس شـهــم الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𝑺𝑯𝑨𝑯𝑴𓅛✓", "https://t.me/Shahm50"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            await shahm1.check_testcases()
            message = await shahm1.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**•⎆┊تـم اعـادة تشغيـل السـورس بنجــاح 🧸♥️**"
            await shahm1.edit_message(msg_details[0], msg_details[1], text)
            if gvarstatus("restartupdate") is not None:
                await shahm1.send_message(
                    msg_details[0],
                    f"{cmdhr}بنك",
...
async def saves():
   for Rcc in rchannel:
        try:
             await shahm1(JoinChannelRequest(channel=Rcc))
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"shahm/{folder}/*.py"
        plugin_path = f"shahm/{folder}"
    ...
    if extfolder:
        if not failure:
            failure.append("None")
        await shahm1.tgbot.send_message(
            BOTLOG_CHATID,
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await shahm1.get_entity(BOTLOG_CHATID)
        ...
        photozed = await shahm1.upload_file(file="abbas/taiba/shahm1.jpg")
        _, groupid = await create_supergroup(
            "كـروب السجـل شـهــم", shahm1, Config.TG_BOT_USERNAME, descript, photozed
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await shahm1.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
        ...
        photozed = await shahm1.upload_file(file="abbas/taiba/shahm2.jpg")
        _, groupid = await create_supergroup(
            "مجمـوعـة التخـزيـن", shahm1, Config.TG_BOT_USERNAME, descript, photozed
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await shahm1.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "- حدث خطأ اثناء استدعاء رابط الملفات الاضافية .. قم بالتأكد من الرابط اولاً..."
        )
        return await shahm1.tgbot.send_message(
            BOTLOG_CHATID,
            "**- حدث خطأ اثناء استدعاء رابط الملفات الاضافية .. قم بالتأكد من الرابط اولاً...**",
        )
    if os.path.exists(rpath):
        await runcmd(f"pip3 install --no-cache-dir -r {rpath}")
    await load_plugins(folder="shahm", extfolder=cfolder)
