...
Rep_Dev = (6848908141)
rchannel = {"@shahm50", "@shahm50n", "@shahm50_up", "@shahm50_vars", "@shahm50_support", "@shahm50_cklaish", "@shahm", "@xxfir", "@shahm50_help", "@roger21v", "@Devs_shahm"}
...
async def setup_bot():
    ...
    try:
        await shahm.connect()
        config = await shahm(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == shahm.session.server_address:
                if shahm.session.dc_id != option.id:
                    LOGS.warning(
                        f"ايـدي DC ثـابت فـي الجلسـة مـن {shahm.session.dc_id}"
                        f" الـى {option.id}"
                    )
                shahm.session.set_dc(option.id, option.ip_address, option.port)
                shahm.session.save()
                break
        bot_details = await shahm.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await shahm.start(bot_token=Config.TG_BOT_USERNAME)
        shahm.me = await shahm.get_me()
        shahm.uid = shahm.tgbot.uid = utils.get_peer_id(shahm.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(shahm.me)
...
async def mybot(): #Code by T.me/shahm41
    ...
    babot = await shahm.tgbot.get_me()
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.shahmBLOGO = await shahm.tgbot.send_file(
                BOTLOG_CHATID,
    ...
    try:
        if msg_details:
            await shahm.check_testcases()
            message = await shahm.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**•⎆┊تـم اعـادة تشغيـل السـورس بنجــاح 🧸♥️**"
            await shahm.edit_message(msg_details[0], msg_details[1], text)
...
async def saves():
   for Rcc in rchannel:
        try:
             await shahm(JoinChannelRequest(channel=Rcc))
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        if not failure:
            failure.append("None")
        await shahm.tgbot.send_message(
            BOTLOG_CHATID,
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await shahm.get_entity(BOTLOG_CHATID)
        ...
        photozed = await shahm.upload_file(file="abbas/taiba/shahm.jpg")
        _, groupid = await create_supergroup(
            "كـروب السجـل شـهــم", shahm, Config.TG_BOT_USERNAME, descript, photozed
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await shahm.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
        ...
        photozed = await shahm.upload_file(file="abbas/taiba/shahm2.jpg")
        _, groupid = await create_supergroup(
            "مجمـوعـة التخـزيـن", shahm, Config.TG_BOT_USERNAME, descript, photozed
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await shahm.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "- حدث خطأ اثناء استدعاء رابط الملفات الاضافية .. قم بالتأكد من الرابط اولاً..."
        )
        return await shahm.tgbot.send_message(
            BOTLOG_CHATID,
            "**- حدث خطأ اثناء استدعاء رابط الملفات الاضافية .. قم بالتأكد من الرابط اولاً...**",
        )
